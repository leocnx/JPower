package top.auok.cbps.persistence.orm.jpa;
...
public class BaseDAO<T, ET extends T, K> implements DataAccessObject<T, K> {

	protected Class<ET> entityClass;

	protected BaseDAO(Class<ET> entityClass) {
	...
	private Map<String, Supplier<Join<?, ?>>> mergeFrom(Root<ET> root,
			BiFunction<CriteriaBuilder, Root<ET>, Map<String, Supplier<Join<?, ?>>>> joinProvider,
			BiFunction<CriteriaBuilder, Root<ET>, Map<String, Supplier<Fetch<?, ?>>>> fetchSupplier,
			CriteriaBuilder cb) {
		final Map<String, Supplier<Join<?, ?>>> allJoins = new HashMap<>();
		Optional.ofNullable(joinProvider.apply(cb, root)).ifPresent(allJoins::putAll);
		if (fetchSupplier != null) {
			Optional.ofNullable(fetchSupplier.apply(cb, root)).ifPresent(m -> allJoins.putAll((Map) m));
		}
		return allJoins;
	}
	...
	private long doFindAllWhere(Collection<? super T> receivingList,
		...
		final TypedQuery<ET> q = em.createQuery(query);
		receivingList.addAll(q.setFirstResult(startIndex).setMaxResults(resultsNumber).getResultList());
